...
public class PayOrderUseCase extends
        UseCase<PayOrderUseCase.RequestValues, PayOrderUseCase.ResponseValue> {

    private final OrdersDataSource mDataSource;

    public PayOrderUseCase(@NonNull OrdersDataSource dataSource,
    ...
    protected Observable<ResponseValue> executeUseCase(RequestValues requestValues) {
        ...
        // #1. create order
        return mDataSource.payOrder(requestValues.getOrderSerial())
                .map(
