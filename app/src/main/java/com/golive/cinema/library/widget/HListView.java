...
package com.golive.cinema.library.widget;
...
/**
 * A view that shows items in a vertically scrolling list. The items come from the {@link
...
public class HListView extends AbsHListView {
    ...
    /**
     * When arrow scrolling, ListView will never scroll more than this factor times the height of
     * the list.
     */
    private static final float MAX_SCROLL_FACTOR = 0.33f;

    /**
     * When arrow scrolling, need a certain amount of pixels to preview next items. This is usually
     * the fading edge, but if that is
     ...
     * with one that will also account
     ...
     * once, the views will appear in the
     * order they were added. Views added using this call can take focus if they want.
     * <p>
     * NOTE: Call this before calling setAdapter. This is so ListView can wrap the supplied cursor
     * with one that will also account
     ...
     * on the ListView features currently
     ...
     *                list and for producing a view to
     ...
     *
     * @param pos       The first position to put in the list
     * @param nextRight The location where the right of the item associated with pos should be
     *                  drawn
    ...
    /**
     * Put mSelectedPosition in the middle of the screen and then build up and down from there. This
     * method forces mSelectedPosition
     * to the center.
     *
     ...
     * may be moved so that it does not
     ...
     */
    private int getRightSelectionPixel(int childrenRight, int fadingEdgeLength, int
            selectedPosition) {
     ...
     */
    private int getLeftSelectionPixel(int childrenLeft, int fadingEdgeLength, int
            selectedPosition) {
    ...
    /**
     * Smoothly scroll to the specified adapter position offset. The view will scroll such that the
     * indicated position is displayed.
     *
    ...
    /**
     * Fills the list based on positioning the new selection relative to the old selection. The new
     * selection will be placed at,
     * above, or below the location of the new selection depending on how the selection is moving.
     * The selection will then be pinned
     * to the visible part of the screen, excluding the edges that are faded. The list is then
     * filled upwards and downwards from
     * there.
     *
     * @param oldSel        The old selected view. Useful for trying to put the new selection in
     *                      the
     *                      same place
     * @param newSel        The view that is to become selected. Useful for trying to put the new
     ...
     * @return The view that currently has selection
     */
    private View moveSelection(View oldSel, View newSel, int delta, int childrenLeft, int
            childrenRight) {
        int fadingEdgeLength = getHorizontalFadingEdgeLength();
        final int selectedPosition = mSelectedPosition;
        ...
        if (delta > 0) {
            ...
            oldSel = makeAndAddView(selectedPosition - 1, oldSel.getLeft(), true, mListPadding.top,
                    false);
			...
			/*
			 * Before After | | | | +-------+ +-------+ | A | | A | +-------+ => | 1 | | B |
			 * +-------+ | 2 | | B | +-------+ +-------+
			 * | | | |
			 *
			 * Try to keep the top of the item about to become selected where it was. newSel = A
			 * olSel = B
			 */
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.UNSPECIFIED) {
            ...
            int[] result = measureWithLargeChildren(heightMeasureSpec, mMeasureWithChild,
                    mMeasureWithChild, widthSize, heightSize, -1);
            heightSize = result[1];
    ...
    /**
     * @return True to recycle the views used to measure this ListView in UNSPECIFIED/AT_MOST modes,
     * false otherwise.
     * @hide
     */
     ...
     * this ListView's padding and divider
     * heights included. If maxHeight is provided, the measuring will stop when the current height
     * reaches maxHeight.
     *
     * @param heightMeasureSpec            The height measure spec to be given to a child's {@link
     *                                     View#measure(int, int)}.
     * @param startPosition                The position of the first child to be shown.
     * @param endPosition                  The (inclusive) position of the last child to be shown.
     *                                     Specify {@link #NO_POSITION} if the last child should be
     *                                     the
     ...
     *                                     allowed. Example: it looks nice to have at least 3
     *                                     completely visible children, and in portrait this will
     *                                     most likely fit; but in landscape there could be times
     *                                     when
     *                                     even 2 children can not be completely shown, so a value
    ...
    final int[] measureWithLargeChildren(int heightMeasureSpec, int startPosition, int endPosition,
        ...
        if (LOG_ENABLED) {
            Log.i(LOG_TAG,
                    "measureWithLargeChildren, from " + startPosition + " to " + endPosition);
        }
    ...
    /**
     * Check if we have dragged the right of the list too wide (we have pushed the left element off
     * the left of the screen when we did
    ...
    /**
     * Check if we have dragged the right of the list too low (we have pushed the right element off
     * the right of the screen when
    ...
    protected void layoutChildren() {
        ...
        try {
			...
			/*
			 * // Remember which child, if any, had accessibility focus. final ViewRootImpl
			 * viewRootImpl = getViewRootImpl(); if (
			 * viewRootImpl != null ) { final View accessFocusedView = viewRootImpl
			 * .getAccessibilityFocusedHost(); if (
			 * accessFocusedView != null ) { final View accessFocusedChild =
			 * findAccessibilityFocusedChild( accessFocusedView ); if (
			 * accessFocusedChild != null ) { if ( !dataChanged || isDirectChildHeaderOrFooter(
			 * accessFocusedChild ) ) { // If the
			 * views won't be changing, try to maintain // focus on the current view host and (if
			 * // applicable) its virtual view.
			 * accessibilityFocusLayoutRestoreView = accessFocusedView;
			 * accessibilityFocusLayoutRestoreNode = viewRootImpl
			 * .getAccessibilityFocusedVirtualView(); } else { // Otherwise, try to maintain focus
            ...
            // accessibilityFocusLayoutRestoreNode.performAction(AccessibilityNodeInfo
            ...
            // final int position = MathUtils.constrain( ( accessibilityFocusPosition -
            // mFirstPosition ), 0, ( getChildCount() - 1 ) );
    ...
    /**
     * Obtain the view and add it to our list of children. The view can be made fresh, converted
     * from an unused view, or used as is
     ...
     */
    private View makeAndAddView(int position, int x, boolean flow, int childrenTop, boolean
            selected) {
        View child;
     ...
     * @param childrenTop Top edge where children should be positioned
     * @param selected    Is this position selected?
     * @param recycled    Has this view been pulled from the recycle bin? If so it does not need to
     *                    be remeasured.
     */
    @TargetApi(11)
    private void setupChild(View child, int position, int x, boolean flowDown, int childrenTop,
        ...
        final boolean needToMeasure = !recycled || updateChildSelected || child
                .isLayoutRequested();
        ...
        if (android.os.Build.VERSION.SDK_INT >= 11) {
            if (recycled
                    && (((AbsHListView.LayoutParams) child.getLayoutParams()).scrappedFromPosition)
                    != position) {
     ...
     * still be positioned
     * appropriately. If the specified selection position is less than 0, then the item at position
     * 0 will be selected.
     *
    ...
    /**
     * Sets the selected item and positions the selection x pixels from the left edge of the
     * ListView. (If in touch mode, the item
     * will not be selected but it will still be positioned appropriately.)
     *
     * @param position Index (starting at 0) of the data item to be selected.
     * @param x        The distance from the left edge of the ListView (plus padding) that the item
     *                 will be positioned.
     */
    ...
    /**
     * setSelectionAfterHeaderView set the selection to be the first list item after the header
     * views.
     */
     ...
     * within the internal focus of the
    ...
    /**
     * To avoid horizontal focus searches changing the selected item, we manually focus search
     * within the selected item (as
     ...
     * whether there are focusable items
     * etc.
     *
     * @param direction Either {@link android.view.View#FOCUS_UP} or {@link
    ...
    private boolean arrowScrollImpl(int direction) {
        ...
        if (nextSelectedPosition != INVALID_POSITION) {
            handleNewSelectionChange(selectedView, direction, nextSelectedPosition,
                    focusResult != null);
        ...
        if (nextSelectedPosition == INVALID_POSITION && selectedView != null && !isViewAncestorOf(
                selectedView, this)) {
            selectedView = null;
            hideSelector();
     ...
     * will change its size. If so, we
     * need to offset some folks, and re-layout the items as appropriate.
     *
     * @param selectedView        The currently selected view (before changing selection). should
     *                            be
     *                            <code>null</code> if there was no previous
     *                            selection.
     * @param direction           Either {@link android.view.View#FOCUS_UP} or {@link
     *                            android.view.View#FOCUS_DOWN}.
     ...
     *                            something has focus, we don't want to show selection
     *                            (ugh).
     */
    private void handleNewSelectionChange(View selectedView, int direction, int
            newSelectedPosition,
        ...
        if (leftView != null) {
            ...
            if (mOnViewSelectedChangeListener != null) {
                mOnViewSelectedChangeListener.OnViewSelectedChange(leftViewIndex, leftView,
                        leftView.isSelected());
            }
        ...
        if (rightView != null) {
            ...
            if (mOnViewSelectedChangeListener != null) {
                mOnViewSelectedChangeListener.OnViewSelectedChange(rightViewIndex, rightView,
                        rightView.isSelected());
            }
            measureAndAdjustRight(rightView, rightViewIndex, numChildren);
        }
    }

    public OnViewSelectedChangeListener mOnViewSelectedChangeListener;

    public static interface OnViewSelectedChangeListener {
        void OnViewSelectedChange(int position, View view, boolean selected);
    }

    public OnViewSelectedChangeListener getOnViewSelectedChangeListener() {
        return mOnViewSelectedChangeListener;
    }

    public void setOnViewSelectedChangeListener(
            OnViewSelectedChangeListener mOnViewSelectedChangeListener) {
        this.mOnViewSelectedChangeListener = mOnViewSelectedChangeListener;
    }
    ...
    /**
     * Layout a child that has been measured, preserving its top position. TODO: unify with
     * setUpChild.
     *
     * @param child The child.
     */
    ...
    /**
     * Determine how much we need to scroll in order to get the next selected view visible, with a
     * fading edge showing below as
    ...
    static private class ArrowScrollFocusResult {
        ...
        /**
         * How {@link com.golive.cinema.library.widget.HListView#arrowScrollFocused} returns its
         * values.
         */
     ...
     * that there might be no selection. Returns {@link #INVALID_POSITION} if there is no
     * selectable
     * view on screen in the
     * given direction.
     */
     ...
     * selection delta and amount to scroll via
    ...
    /**
     * Scroll the children by amount, adding a view at the end and removing views that fall off as
     * necessary.
     *
    ...
    private View addViewBefore(View theView, int position) {
        ...
        setupChild(view, abovePosition, edgeOfNewChild, false, mListPadding.top, false,
                mIsScrap[0]);
        return view;
    }
    ...
    public boolean isOpaque() {
        ...
        if (retValue) {
            ...
            final int listRight =
                    getWidth() - (mListPadding != null ? mListPadding.right : getPaddingRight());
     ...
     * @param childIndex The index of child (of the View) above the divider. This will be -1 if
     *                   there is no child above the divider to be
     *                   drawn.
     */
    ...
    /**
     * Sets the height of the divider that will be drawn between each item in the list. Calling this
     * will override the intrinsic
    ...
    /**
     * Sets the drawable that will be drawn above all other list content. This area can become
     * visible when the user overscrolls the
     * list.
     *
    ...
    /**
     * Sets the drawable that will be drawn below all other list content. This area can become
     * visible when the user overscrolls the
	 ...
	 * (non-Javadoc)
	 *
	 * Children specified in XML are assumed to be header views. After we have parsed them move
	 * them out of the children list and
	 * into mHeaderViews.
	 */
    @Override
	...
	/*
	 * @Override protected View findViewByPredicateTraversal( Predicate<View> predicate, View
	 * childToSkip ) { View v; v =
	 * super.findViewByPredicateTraversal( predicate, childToSkip ); if ( v == null ) { v =
	 * findViewByPredicateInHeadersOrFooters(
	 * mHeaderViewInfos, predicate, childToSkip ); if ( v != null ) { return v; }
	 *
	 * v = findViewByPredicateInHeadersOrFooters( mFooterViewInfos, predicate, childToSkip ); if (
	  * v != null ) { return v; } } return
	 * v; }
	 */

	/*
	 * View findViewByPredicateInHeadersOrFooters( ArrayList<FixedViewInfo> where, Predicate<View>
	  *     predicate, View childToSkip ) { if
