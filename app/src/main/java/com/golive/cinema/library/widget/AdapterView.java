...
public abstract class AdapterView<T extends Adapter> extends ViewGroup {
    ...
    /**
     * The item view type returned by {@link Adapter#getItemViewType(int)} when the adapter does
     * not
    ...
    /**
     * The item view type returned by {@link Adapter#getItemViewType(int)} when the item is a
     * header
     * or footer.
     */
    ...
    /**
     * The offset in pixels from the left of the AdapterView to the left of the view to select
    ...
    /**
     * Indicates what focusable state is requested when calling setFocusable(). In addition to
     * this,
     * this view has other criteria for
     * actually determining the focusable state (such as whether its empty or the text filter is
     * shown).
     *
     ...
     * is used to layout the children
    ...
    /**
     * @return The callback to be invoked with an item in this AdapterView has been clicked, or
     * null
     ...
     * @param position The position of the view in the adapter.
     * @param id       The col id of the item that was clicked.
     * @return True if there was an assigned OnItemClickListener that was called, false
     * otherwise is
     * returned.
     */
     ...
     * held, or null id no callback as been
     * set.
     */
    ...
    public interface OnItemSelectedListener {
         ...
         * is invoked only when the newly
         * selected position is different from the previously selected position or if there was no
         * selected item.
         * </p>
         *
         ...
         * selection
         * can disappear for instance when
     ...
     * ContextMenuInfo) } callback when a
    ...
    public void removeView(View child) {
        throw new UnsupportedOperationException("removeView(View) is not supported in "
                + "AdapterView");
    }
     ...
     * the number of data items, which
     ...
     * or a descendant of an adapter
     * item.
     *
     * @param view an adapter item, or a descendant of an adapter item. This must be visible in
     *             this
     *             AdapterView at the time of the
     *             call.
     * @return the position within the adapter's data set of the view, or {@link #INVALID_POSITION}
     * if the view does not correspond
     ...
     * method must invoke the overriden
     * super method first.
     *
    ...
    public void setEmptyView(View emptyView) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 16) {
            // If not explicitly specified this view is important for accessibility.
            if (emptyView != null && emptyView.getImportantForAccessibility() ==
     ...
     * view. The empty view is used to
     ...
     * empty view, display it. In all the
     * other cases, make sure that the listview is VISIBLE and that the empty view is GONE (if it's
     * not null).
     */
     ...
     * and then alternates between moving
     * up and moving down until 1) we find the right position, or 2) we run out of time, or 3) we
     * have looked at every position
     *
