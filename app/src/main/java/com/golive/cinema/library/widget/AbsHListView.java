...
import com.golive.cinema.library.util.ViewHelperFactory;
import com.golive.cinema.library.util.ViewHelperFactory.ViewHelper;
import com.golive.cinema.library.util.v11.MultiChoiceModeListener;
import com.golive.cinema.library.util.v11.MultiChoiceModeWrapper;
...
public abstract class AbsHListView extends AdapterView<ListAdapter> implements
     ...
     */
    protected final Rect mSelectorRect = new Rect();
    ...
    public interface SelectionBoundsAdjuster {
         ...
         *               is the bounds of the entire view). The
    ...
    public AbsHListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray array = theme.obtainStyledAttributes(attrs, R.styleable.AbsHListView,
                defStyle, 0);
        ...
        if (null != array) {
            ...
            drawSelectorOnTop = array.getBoolean(R.styleable.AbsHListView_android_drawSelectorOnTop,
                    false);
            ...
            transcriptMode = array.getInt(R.styleable.AbsHListView_hlv_transcriptMode,
                    TRANSCRIPT_MODE_DISABLED);
            color = array.getColor(R.styleable.AbsHListView_android_cacheColorHint, 0);
            smoothScrollbar = array.getBoolean(R.styleable.AbsHListView_android_smoothScrollbar,
                    true);
            choiceMode = array.getInt(R.styleable.AbsHListView_android_choiceMode,
                    ListView.CHOICE_MODE_NONE);
            array.recycle();

            if (LOG_ENABLED) {
                Log.d(TAG, "listSelector: " + listSelector);
                Log.d(TAG, "cacheColorHint: " + color);
     ...
     *
     * @return The position of the currently checked item or {@link #INVALID_POSITION} if
     * nothing is
     * selected
     * @see #setChoiceMode(int)
     */
    public int getCheckedItemPosition() {
        if (mChoiceMode == ListView.CHOICE_MODE_SINGLE && mCheckStates != null && mCheckStates
                .size() == 1) {
    ...
    /**
     * Returns the set of checked items in the list. The result is only valid if the choice mode
     * has
     * not been set to
    ...
    public long[] getCheckedItemIds() {
        if (mChoiceMode == ListView.CHOICE_MODE_NONE || mCheckedIdStates == null || mAdapter ==
                null) {
            return new long[0];
        }
                ...
                if (mMultiChoiceModeCallback == null ||
                    ...
                    throw new IllegalStateException(
                            "AbsListView: attempted to start selection mode " +
                                    "for CHOICE_MODE_MULTIPLE_MODAL but no choice mode callback "
                                    + "was "
                                    +
                                    "supplied. Call setMultiChoiceModeListener to set a callback.");
                }
                mChoiceActionMode = startActionMode((MultiChoiceModeWrapper)
                        mMultiChoiceModeCallback);
            }
        }

        if (mChoiceMode == ListView.CHOICE_MODE_MULTIPLE
                || (android.os.Build.VERSION.SDK_INT >= 11 && mChoiceMode == ListView
            ...
            if (mChoiceActionMode != null) {
                ...
                ((MultiChoiceModeWrapper) mMultiChoiceModeCallback).onItemCheckedStateChanged(
                        (ActionMode) mChoiceActionMode, position, id, value);
            }
        } else {
    ...
    public boolean performItemClick(View view, int position, long id) {
        ...
        if (mChoiceMode != ListView.CHOICE_MODE_NONE) {
            ...
            if (mChoiceMode == ListView.CHOICE_MODE_MULTIPLE
                ...
                if (mChoiceActionMode != null) {
                    ((MultiChoiceModeWrapper) mMultiChoiceModeCallback).onItemCheckedStateChanged(
                            (ActionMode) mChoiceActionMode, position, id, checked);
    ...
    /**
     * When stack from bottom is set to true, the list fills its content starting from the
     * bottom of
     * the view.
     *
    ...
    public Parcelable onSaveInstanceState() {
         ...
         * This doesn't really make sense as the place to dismiss the popups, but there don't seem
		  * to be any other useful hooks that
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 11) {
            if (ss.inActionMode && mChoiceMode == ListView.CHOICE_MODE_MULTIPLE_MODAL
                ...
                mChoiceActionMode = startActionMode(
	...
	/*
     * @Override protected boolean setFrame( int left, int top, int right, int bottom ) { final
	 * boolean changed = super.setFrame(
	 * left, top, right, bottom );
	 *
	 * if ( changed ) { // Reposition the popup when the frame has changed. This includes //
	 * translating the widget, not just
	 * changing its dimension. The // filter popup needs to follow the widget. final boolean
	 * visible = getWindowVisibility() ==
	 * View.VISIBLE; if ( mFiltered && visible && mPopup != null && mPopup.isShowing() ) {
	 * positionPopup(); } }
	 *
	 * return changed; }
	 */
     ...
     * @param isScrap  Array of at least 1 boolean, the first entry will become true if the
     *                 returned
     *                 view was taken from the scrap heap,
     *                 false if otherwise.
    ...
    protected void dispatchDraw(Canvas canvas) {
        /*
         * TODO: check this final boolean clipToPadding = (mGroupFlags & CLIP_TO_PADDING_MASK) ==
		 * CLIP_TO_PADDING_MASK; if
		 * (clipToPadding) { saveCount = canvas.save(); final int scrollX = mScrollX; final int
		 * scrollY = mScrollY;
		 * canvas.clipRect(scrollX + mPaddingLeft, scrollY + mPaddingTop, scrollX + mRight - mLeft
		  * - mPaddingRight, scrollY + mBottom
		...
		/*
		 * TODO: check this if (clipToPadding) { canvas.restoreToCount(saveCount); mGroupFlags |=
		 * CLIP_TO_PADDING_MASK; }
		 */
    }

	/*
	 * @Override protected boolean isPaddingOffsetRequired() { return ( mGroupFlags &
	 * CLIP_TO_PADDING_MASK ) != CLIP_TO_PADDING_MASK;
	 * }
	 *
	 * @Override protected int getLeftPaddingOffset() { return ( mGroupFlags &
	 * CLIP_TO_PADDING_MASK ) == CLIP_TO_PADDING_MASK ? 0 :
	 * -mPaddingLeft; }
	 *
	 * @Override protected int getTopPaddingOffset() { return ( mGroupFlags & CLIP_TO_PADDING_MASK
	  * ) == CLIP_TO_PADDING_MASK ? 0 :
	 * -mPaddingTop; }
	 *
    ...
    boolean performLongPress(final View child, final int longPressPosition,
        ...
        if (android.os.Build.VERSION.SDK_INT >= 11) {
            if (mChoiceMode == ListView.CHOICE_MODE_MULTIPLE_MODAL) {
                if (mChoiceActionMode == null &&
                        (mChoiceActionMode = startActionMode(
                                (MultiChoiceModeWrapper) mMultiChoiceModeCallback)) != null) {
...
//		return showContextMenu( x, y, metaState );
        ...
        if (mTouchMode == TOUCH_MODE_SCROLL) {
            ...
            if (x != mLastX) {
                ...
                if (motionView != null) {
                    ...
                    if (atEdge) {
                        ...
                        overScrollBy(overscroll, 0, getScrollX(), 0, 0, 0, mOverscrollDistance, 0,
                                true);
            ...
            if (x != mLastX) {
                ...
                if (overScrollDistance != 0) {
                    overScrollBy(overScrollDistance, 0, getScrollX(), 0, 0, 0, mOverscrollDistance,
                            0, true);
                    final int overscrollMode = getOverScrollMode();
                    if (overscrollMode == OVER_SCROLL_ALWAYS || (
                        ...
                        if (childCount > 0) {
                            ...
                            if (mFirstPosition == 0 && firstChildLeft >= contentLeft
                                ...
                                final int initialVelocity = (int) (velocityTracker.getXVelocity(
                                        mActivePointerId) * mVelocityScale);
    ...
    /**
     * Fires an "on scroll state changed" event to the registered {@link
     * com.golive.cinema.library.widget.AbsHListView.OnScrollListener}, if any.
    ...
    private class FlingRunnable implements Runnable {
         ...
         */
        private final com.golive.cinema.library.widget.OverScroller mScroller;
        ...
        private final Runnable mCheckFlywheel = new Runnable() {
            ...
            public void run() {
                ...
                if (Math.abs(xvel) >= mMinimumVelocity && scroller.isScrollingInDirection(xvel,
                        0)) {
        ...
        void startOverfling(int initialVelocity) {
            mScroller.setInterpolator(null);
            mScroller.fling(getScrollX(), 0, initialVelocity, 0, Integer.MIN_VALUE,
                    Integer.MAX_VALUE, 0, 0, getWidth(), 0);
            mTouchMode = TOUCH_MODE_OVERFLING;
            invalidate();
            mViewHelper.postOnAnimation(this);
        }
            ...
            switch (mTouchMode) {
                ...
                case TOUCH_MODE_FLING: {
                    ...
                    if (delta > 0) {
                        ...
                        delta = Math.min(getWidth() - getPaddingRight() - getPaddingLeft() - 1,
                                delta);
                    } else {
                        ...
                        // Don't fling more than 1 screen
                        delta = Math.max(-(getWidth() - getPaddingRight() - getPaddingLeft() - 1),
                                delta);
                    }
                    ...
                    if (atEnd) {
                        if (motionView != null) {
                            ...
                            overScrollBy(overshoot, 0, getScrollX(), 0, 0, 0, mOverflingDistance, 0,
                                    false);
                        }
                        if (more) {
                            edgeReached(delta);
                ...
                case TOUCH_MODE_OVERFLING: {
                    ...
                    if (scroller.computeScrollOffset()) {
                        ...
                        if (overScrollBy(deltaX, 0, scrollX, 0, 0, 0, mOverflingDistance, 0,
                                false)) {
        ...
        public void run() {
            ...
            switch (mMode) {
                case MOVE_DOWN_POS: {
                    ...
                    final int extraScroll = lastPos < mItemCount - 1 ? Math.max(mListPadding.right,
                            mExtraScroll) : mListPadding.right;
                ...
                case MOVE_OFFSET: {
                    ...
                    if (position < firstPos) {
                        ...
                        final int duration = (int) (mScrollDuration * ((float) Math.abs(distance)
                                / getWidth()));
                        smoothScrollBy(distance, duration, true);
                    }
                    break;
     ...
     *
     * @param deltaY            Amount to offset mMotionView. This is the accumulated delta since
     *                          the motion began. Positive numbers mean the user's
    ...
    boolean trackMotionScroll(int deltaX, int incrementalDeltaX) {
		...
		/*
		 * TODO: check this if ((mGroupFlags & CLIP_TO_PADDING_MASK) == CLIP_TO_PADDING_MASK) {
                        ...
                        if (mChoiceActionMode != null && mMultiChoiceModeCallback != null) {
                            ((MultiChoiceModeWrapper) mMultiChoiceModeCallback)
                                    .onItemCheckedStateChanged(
                                            (ActionMode) mChoiceActionMode, lastPos, id, false);
                        }
                    }
                }
            } else {
     ...
     * @see com.golive.cinema.library.widget.AbsHListView.RecycleBin
     * @see com.golive.cinema.library.widget.AbsHListView.RecyclerListener
     */
    ...
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        /**
         * View type for this view, as returned by {@link android.widget.Adapter#getItemViewType
         * (int)
         * }
         */
        public int viewType;
     ...
     *
     * @see com.golive.cinema.library.widget.AbsHListView.RecycleBin
     * @see com.golive.cinema.library.widget.AbsHListView#setRecyclerListener(com.golive.cinema
     * .library.widget.AbsHListView.RecyclerListener)
     */
    ...
    public class RecycleBin {
        ...
        public void addScrapView(View scrap, int position) {
            ...
            final boolean scrapHasTransientState =
                    android.os.Build.VERSION.SDK_INT >= 16 ? scrap.hasTransientState() : false;
        ...
        public void scrapActiveViews() {
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                if (victim != null) {
                    ...
                    final boolean scrapHasTransientState = android.os.Build.VERSION.SDK_INT >= 16
                            ? victim.hasTransientState() : false;
    ...
    static View retrieveFromScrap(ArrayList<View> scrapViews, int position) {
        ...
        if (size > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (((AbsHListView.LayoutParams) view.getLayoutParams()).scrappedFromPosition
                        == position) {
                    scrapViews.remove(i);
                    return view;
                }
