 ...
 * differs from the {@link com.golive.cinema.library.widget.HListView} by allowing two levels:
 * groups which can
...
public class ExpandableHListView extends HListView {
    ...
    private final int mIndicatorTop;

    /** Right bound for drawing the indicator. */
    private final int mIndicatorLeft;

    /** Group indicator gravity */
    private final int mIndicatorGravity;

    /** Child indicator gravity */
    private final int mChildIndicatorGravity;

    private final int mChildIndicatorTop;

    private final int mChildIndicatorLeft;
    ...
    public ExpandableHListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ExpandableHListView,
                defStyle, 0);
        ...
        mIndicatorGravity = a.getInt(R.styleable.ExpandableHListView_hlv_indicatorGravity,
                Gravity.NO_GRAVITY);
        mChildIndicatorGravity = a.getInt(R.styleable.ExpandableHListView_hlv_childIndicatorGravity,
                Gravity.NO_GRAVITY);

        mChildIndicatorLeft = a.getDimensionPixelSize(
                R.styleable.ExpandableHListView_hlv_childIndicatorPaddingLeft, 0);
        mChildIndicatorTop = a.getDimensionPixelSize(
                R.styleable.ExpandableHListView_hlv_childIndicatorPaddingTop, 0);
        ...
        for (int i = 0, childFlPos = mFirstPosition - headerViewsCount; i < childCount;
            ...
            if (indicatorRect.top != indicatorRect.bottom) {
                ...
                if (indicator != null) {
                    ...
                    if (pos.position.type == ExpandableHListPosition.CHILD) {
                        Gravity.apply(mChildIndicatorGravity, mChildIndicatorWidth,
                                mChildIndicatorHeight, indicatorRect, mTempRect);
                    } else {
    ...
    private Drawable getIndicator(ExpandableHListConnector.PositionMetadata pos) {
        ...
        if (pos.position.type == ExpandableHListPosition.GROUP) {
            ...
            if (indicator != null && indicator.isStateful()) {
                ...
                boolean isEmpty = (pos.groupMetadata == null) || (pos.groupMetadata.lastChildFlPos
    ...
    void drawDivider(Canvas canvas, Rect bounds, int childIndex) {
        ...
        if (flatListPosition >= 0) {
            ...
            ExpandableHListConnector.PositionMetadata pos = mConnector.getUnflattenedPos(
                    adjustedPosition);
            // If this item is a child, or it is a non-empty group that is expanded
            if ((pos.position.type == ExpandableHListPosition.CHILD) || (pos.isExpanded()
    ...
    boolean handleItemClick(View v, int position, long id) {
        final ExpandableHListConnector.PositionMetadata posMetadata = mConnector.getUnflattenedPos
                (position);
        ...
        if (posMetadata.position.type == ExpandableHListPosition.GROUP) {
            ...
            if (mOnGroupClickListener != null) {
                if (mOnGroupClickListener.onGroupClick(this, v, posMetadata.position.groupPos,
                        id)) {
                    posMetadata.recycle();
                    return true;
                }
    ...
    public interface OnChildClickListener {
        ...
        boolean onChildClick(
                ExpandableHListView parent, View v, int groupPosition, int childPosition, long
                id);
    }
    ...
    public long getExpandableListPosition(int flatListPosition) {
        ...
        final int adjustedPosition = getFlatPositionForConnector(flatListPosition);
        ExpandableHListConnector.PositionMetadata pm = mConnector.getUnflattenedPos(
                adjustedPosition);
    ...
    public int getFlatListPosition(long packedPosition) {
        ExpandableHListPosition elPackedPos = ExpandableHListPosition.obtainPosition(
                packedPosition);
    ...
    public void setSelectedGroup(int groupPosition) {
        ExpandableHListPosition elGroupPos = ExpandableHListPosition.obtainGroupPosition(
                groupPosition);
        ExpandableHListConnector.PositionMetadata pm = mConnector.getFlattenedPos(elGroupPos);
        elGroupPos.recycle();
     ...
     */
    public boolean setSelectedChild(int groupPosition, int childPosition, boolean
            shouldExpandGroup) {
        ExpandableHListPosition elChildPos = ExpandableHListPosition.obtainChildPosition(
                groupPosition, childPosition);
        ExpandableHListConnector.PositionMetadata flatChildPos = mConnector.getFlattenedPos(
                elChildPos);
    ...
    public static long getPackedPositionForChild(int groupPosition, int childPosition) {
        return (((long) PACKED_POSITION_TYPE_CHILD) << PACKED_POSITION_SHIFT_TYPE) |
                ((((long) groupPosition) & PACKED_POSITION_INT_MASK_GROUP) <<
                        PACKED_POSITION_SHIFT_GROUP) | (childPosition &
    ...
    static class SavedState extends BaseSavedState {
        final ArrayList<ExpandableHListConnector.GroupMetadata> expandedGroupMetadataList;
    ...
    public Parcelable onSaveInstanceState() {
        ...
        return new SavedState(superState, mConnector != null ? mConnector
                .getExpandedGroupMetadataList() : null);
    }
