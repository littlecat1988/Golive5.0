...
import com.golive.cinema.views.metroviews.Lanes;
import com.golive.cinema.views.metroviews.Lanes.LaneInfo;

public class SpannableGridLayoutManager extends GridLayoutManager {
    private int item_extend_width = 0;
    ...
    public SpannableGridLayoutManager(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle, 3, 3);
    }
...
//    }
    ...
    public void setItemExtendWidth(int width) {
        item_extend_width = width;
    }

    private int getChildWidth(int colSpan) {
        return (getLanes().getLaneSize() + item_extend_width) * colSpan;
    }
    ...
    protected void measureChildWithMargins(View child) {
        ...
        if (isVertical()) {
            int parentWidth =
                    getWidth() - getPaddingLeft() - getPaddingRight() - decorated * (laneCount - 1);
            laneSize = parentWidth / laneCount;
        } else {
            int parentHeight =
                    getHeight() - getPaddingTop() - getPaddingBottom() - decorated * (laneCount
                            - 1);
    ...
    public static class LayoutParams extends TvRecyclerView.LayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            TypedArray a = c.obtainStyledAttributes(attrs,
                    R.styleable.TvRecyclerView_SpannableGridViewChild);
            colSpan = Math.max(
                    DEFAULT_SPAN,
                    a.getInt(R.styleable.TvRecyclerView_SpannableGridViewChild_tv_colSpan, -1));
            rowSpan = Math.max(
                    DEFAULT_SPAN,
                    a.getInt(R.styleable.TvRecyclerView_SpannableGridViewChild_tv_rowSpan, -1));
            a.recycle();
        }
